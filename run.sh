...
if [ $1 = "start" ] ; then
	...
	start-stop-daemon --start --oknodo --make-pidfile --pidfile $PID_FOLDER/example.pid --chdir $CAD_DIR/apps/example --background --startas /usr/bin/lua -- start.lua modbus_test

...
if [ $1 = "start" ] ; then
	...
	start-stop-daemon --start --oknodo --make-pidfile --pidfile $PID_FOLDER/app_test.pid --chdir $CAD_DIR/apps/test --background --startas /usr/bin/lua -- start.lua
	start-stop-daemon --start --oknodo --make-pidfile --pidfile $PID_FOLDER/app_test_pub.pid --chdir $CAD_DIR/apps/test_pub --background --startas /usr/bin/lua -- start.lua
	...
	start-stop-daemon --stop --oknodo --pidfile $PID_FOLDER/app_test.pid --retry 5
	start-stop-daemon --stop --oknodo --pidfile $PID_FOLDER/app_test_pub.pid --retry 5

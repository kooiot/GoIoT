...
handlers.on_start = function(app)
	...
	local config = require 'shared.api.config'
	local port_name = config.get(ioname .. '.port') or '/dev/ttyUSB1'
...
local send_message = function (center_number, phone_number, message)
	...
	port:write('AT+CMGF=1\r\n')
		sleep(500)
	--c_n = string.format('%s%s%s','AT+CSCA=\'+86',center_number,'\'\r\n')
	local e,data,size =port:read(128,1000)
	print ('AT+CMGF == the data & size is ', e, data, size, '\r\n')
	c_n = string.format('%s%s%s','AT+CSCA=\'+86','13800100500','\'\r\n')
		sleep(500)
	port:write(c_n, 1000)
		sleep(500)
	local e,data,size =port:read(128,1000)
	print ('AT+CSCA===the data & size is ', e, data, size, '\r\n')
	p_n = string.format('%s%s%s','AT+CMGS=\'+86',phone_number,'\'\r\n')
		sleep(500)
	port:write(p_n,1000)
		sleep(500)
	local e,data,size =port:read(128,1000)
	print ('AT+CMGS ===the data & size is ', e, data, size, '\r\n')
	port:write(message, 1000)
		sleep(500)
	local e,data,size =port:read(128,1000)
	print ('MESSAGE === the data & size is ', e, data, size, '\r\n')
	port:write('\x1a\r\n', 1000)
		sleep(500)
	local e,data,size =port:read(128,1000)
...
--	end
	...
	if i and j then
		...
		if mod == 'send' then
			...
			print("&^&^&******: ", mod)
			sleep(1000)
			send_message('13800100500', '13001143649', 'rerere')
			config.set(ioname.. '.mod', 'OK')
		end
	else
	end
end

...
return function (path, destination, force)
  ...
  if way == 'exit' then
	  ...
	  if (error_code ~= 0) then
		  return false, code[error_code] or "Unknown error encountered while unzipping. code-"..tostring(error_code)
	  end
  else
	  --print("Unzip terminated by signal "..error_code)
	  return false, "Unzip terminated by signal "..error_code
  end

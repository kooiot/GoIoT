...
local function install_app(app, apps_folder, dest_name, downcb)
	local path, err = downcb(app)
	if path then
		return install(path, apps_folder, dest_name, app)
	else
...
return function(app, apps_folder, dest_name, downcb)
	...
	elseif app.type == 'app.io.config' then
		...
		-- install application
		return install_app(dapp, apps_folder, dest_name, downcb)
	else
